...
def generate_sample():
    ...
    actions = np.asarray(rnd_gen.randint(
...
timestep_encoder = TriangularValueEncoding(
    max_value=max_timestep, triangle_span=int(max_timestep / 10))

states_placeholder = tf.placeholder(
    shape=(n_mb, None, n_features), dtype=tf.float32)
actions_placeholder = tf.placeholder(
    shape=(n_mb, None, n_actions), dtype=tf.float32)
...
with tf.variable_scope('reward_redistribution_model', reuse=tf.AUTO_REUSE):
    ...
    time_input = ReshapeLayer(incoming=time_input_layer, shape=(
        n_mb, 1, timestep_encoder.n_nodes_python))
    ...
    def truncated_normal_init(mean, stddev): return \
        lambda *args, **kwargs: tf.truncated_normal(
            mean=mean, stddev=stddev, *args, **kwargs)
...
init_tensors = OrderedDict([
    ...
    ('predictions', tf.zeros(
        [s if s >= 0 else 1 for s in output_layer.get_output_shape()]))
])

# Get initial tensor shapes in tf format
init_shapes = OrderedDict([
    ('time', init_tensors['time'].get_shape()),
    ('lstm_inputs', tensor_shape_with_flexible_dim(
        init_tensors['lstm_inputs'], dim=1)),
    ('lstm_internals', tensor_shape_with_flexible_dim(
        init_tensors['lstm_internals'], dim=1)),
    ('lstm_h', tensor_shape_with_flexible_dim(init_tensors['lstm_h'], dim=1)),
    ('predictions', tensor_shape_with_flexible_dim(
        init_tensors['predictions'], dim=1)),
])
...
def body(time, lstm_inputs, lstm_internals, lstm_h, predictions, *args):
    ...
    state_input_layer.update(tf.expand_dims(
        states_placeholder[:, time], axis=1))

    # Set actions as network input
    action_input_layer.update(tf.cast(tf.expand_dims(
        actions_placeholder[:, time], axis=1), dtype=tf.float32))
    ...
    lstm_layer.c[-1], lstm_layer.h[-1] = lstm_internals[:, -
                                                        1, :, -1], lstm_h[:, -1, :]
    ...
    lstm_internals = tf.concat([lstm_internals,
                               ...
                               axis=1)
    lstm_h = tf.concat(
        [lstm_h, tf.expand_dims(lstm_layer.h[-1], axis=1)], axis=1)

    # Increment time
    time += tf.constant(1, dtype=tf.int32)

    lstm_inputs = tf.concat(
        [lstm_inputs, reward_redistibution_input.out], axis=1)
...
aux_target_1 = tf.concat(
    [rewards_placeholder, tf.zeros_like(rewards_placeholder[:, :9])], axis=1)
aux_target_1 = tf.nn.conv1d(
    aux_target_1, filters=aux_target_1_filter, padding='VALID', stride=1)
aux_target_2 = tf.reduce_sum(
    rewards_placeholder, axis=1) - tf.cumsum(rewards_placeholder, axis=1)
aux_target_3 = tf.cumsum(rewards_placeholder, axis=1)
targets = tf.concat([rewards_placeholder, aux_target_1,
                     aux_target_2, aux_target_3], axis=-1)

# rr_returns['predictions'].shape == (1, 60, 4)
return_prediction = rr_returns['predictions'][0, -1, 0]
true_return = tf.reduce_sum(targets[0, :, 0]) # scalar
...
rr_reg_penalty = regularize(layers=[
                            lstm_layer, output_layer], l1=1e-6, regularize_weights=True, regularize_biases=True)
...
with tf.variable_scope('reward_redistribution_model', reuse=tf.AUTO_REUSE):
    reward_redistibution_input = RNNInputLayer(
        tf.zeros(intgrd_input_shape_per_ts))
...
def body(time, lstm_c, lstm_h, predictions, *args):
    ...
    # Set model input
    reward_redistibution_input.update(
        tf.expand_dims(intgrd_inputs[:, time], axis=1))
...
# the last timestep
#intgrd_pred = ig_returns['predictions'][..., 0] + \
#    ig_returns['predictions'][..., -1]
intgrd_pred = ig_returns['predictions'][..., 0]
...
intgrd_grads = tf.concat(
    [intgrd_grads[:-10], tf.zeros_like(intgrd_grads[:10])], axis=0)
...
intgrd_grads *= tf.clip_by_value(true_return /
                                 (return_prediction + (tf.sign(return_prediction)
                                                       * (tf.sqrt(epsilon_sqr) / 5))),
...
outputpath = os.path.join(
    'results', dt.datetime.now().strftime("%Y-%m-%dT%H-%M-%S"))
        ...
        if episode % 1000 == 0:
            ...
            save_subplots_line_plots(
                images=[lstm_internal_values[:, :, 0], lstm_internal_values[:, :, 1],
                        lstm_internal_values[:, :,
                                             2], lstm_internal_values[:, :, 3],
                        lstm_internal_values[:, :, 4], lstm_h_values[:, :]],
                subfigtitles=['rr_lstm_ig', 'rr_lstm_og', 'rr_lstm_ci',
                              'rr_lstm_fg', 'rr_lstm_c', 'rr_lstm_h'],
                automatic_positioning=True, tight_layout=True,
                filename=os.path.join(outputpath_training, "lstm_internals_ep{}.png".format(episode)))

    if avg_loss < 0.1 and true_ret != 0 and episode > 5000:
        ...
        if 80 < intgrd_reconstruction_perc_value < 120:
            print([(s, a, i) for s, a, i in zip(np.argmax(sample['states'][0, :, :], axis=-1),
                                                np.argmax(
                                                    sample['states'][0], axis=-1) - int(n_features / 2),
                                                intgrd_grads_value)])
            n_plotted += 1
...
feed_dict = {states_placeholder: sample['states'],
             ...
             avg_reward_placeholder: avg_return}
#for i in range(100):
#    loss, true_ret, pred_ret, _ = tf_sess.run([total_loss, true_return, return_prediction, rr_update], feed_dict=feed_dict)
#    print(loss)


#print(tf_sess.run([intgrd_sum], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([targets], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([grads], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([intgrd_prediction_diff], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([intgrd_grads], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([intgrd_pred], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([intgrd_pred], feed_dict=feed_dict)[0][0].shape)
#print(tf_sess.run([lstm_inputs], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([intgrd_inputs], feed_dict=feed_dict)[0].shape)
##print(tf_sess.run([lstm_inputs], feed_dict=feed_dict)[0][0, 0])
#print(tf_sess.run([ig_returns['predictions']], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([ig_returns['predictions']], feed_dict=feed_dict)[0][..., 0].shape)
#print(rr_returns['predictions'])
#print(n_timesteps)
#print(tf_sess.run([reward_redistibution_input], feed_dict=feed_dict))
#print(tf_sess.run([timestep_encoder.encode_value(tf.constant(0, dtype=tf.int32))], feed_dict=feed_dict)[0])
#print(tf_sess.run([timestep_encoder.encode_value(tf.constant(1, dtype=tf.int32))], feed_dict=feed_dict)[0])
#print(tf_sess.run([timestep_encoder.encode_value(tf.constant(2, dtype=tf.int32))], feed_dict=feed_dict)[0])
#print(tf_sess.run([timestep_encoder.encode_value(tf.constant(3, dtype=tf.int32))], feed_dict=feed_dict)[0])
#print(tf_sess.run([lstm_inputs], feed_dict=feed_dict)[0])
#print(tf_sess.run([true_return], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([rr_returns['predictions']], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([targets], feed_dict=feed_dict)[0][0, 0])
#print(tf_sess.run([rr_returns['lstm_inputs']], feed_dict=feed_dict)[0][0, 0])
#print(tf_sess.run([rr_returns['lstm_internals']], feed_dict=feed_dict)[0].shape)
#print(tf_sess.run([rr_returns['predictions']], feed_dict=feed_dict)[0])
#print(tf_sess.run([rr_returns['time']], feed_dict=feed_dict))
#print(tf_sess.run([n_timesteps], feed_dict=feed_dict))

#print(loss)
#print(timestep_encoder.encode_value(tf.constant(0, dtype=tf.int32)))
#print(timestep_encoder.encode_value(tf.constant(1, dtype=tf.int32)))
#print(timestep_encoder.encode_value(tf.constant(0, dtype=tf.int32)))
#print(timestep_encoder.encode_value(tf.constant(1, dtype=tf.int32)))
